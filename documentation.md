...
### 1. Pobierz plik ZIP z interpreterem
...
Opis techniczny języka dostępny po kliknięciu linku obok.: [opis techniczny](technical_description.md)

## Dokumentacja dla użytkownika

### Hello world!
Poniżej umieszczonone są przykładowe programy typu `Hello world!`. Szczegółowe wyjaśnienie poszczególnych instrukcji i komend znajduje się w dalszej części dokumentacji:
- program grający gamę `C-dur` (można ją znać także jako `do-re-mi-fa-sol-la-si-do`)
```
melody main() {
...
```
- program grający dziecięcą melodyjkę "Wlazł kotek na płotek"
```
melody main() {
    SET INSTRUMENT = PIANO;
...
```
- program grający kadencję `C-dur`
```
melody main() {
    SET INSTRUMENT = PIANO;
    SET PACE = 500;
...
2. **Komentarze**\
Komentarz to fragment w kodzie programu, który jest ignorowany przez interpreter - służy do notowania, dołączania wyjaśnień do kodu. Język `Cb` udostępnia 2 rodzaje komentarzy:
...
```
// mogę sobie tutaj napisać co mi się podoba :)
...
/*  PLAY C#2 5000;
    PAUSE 200;
    PLAY B-1 250;   */
// powyższy fragment nie zostanie wykonany - jest częścią komentarza, mimo że to dobry kod w języku Cb
}
```
...
```
melody main() {
    int i;          // defaultowo przyjmie wartość 0
    bool b = false;
    Note n = C2;
    Chord ch;       // będzie nullem - wartością pustą
    ch = [C2, E2, G2,];     // teraz stanie się akordem C-dur
    Track t;        // o tym dalej

    PLAY n n n i;   // odegranie melodii z wykorzystaniem zmiennych
}
```
...
melody main() {
    ...
    PLAY [D2, F2, A2] 1000;     // dźwięk organów będzie zmodyfikowany wg zdefiniowanych efektów
}
```

5. **Komendy PLAY i PAUSE**\
...
- strukturę `Track` - melodię polifoniczną (szersze omówienie w dalszej części dokumentacji)
```
melody main() {
    PLAY C2 E2 G2 500;
    PLAY C3 1000;
...
7. **Deklaracja funkcji main** to szczególny przypadek deklaracji funkcji, musi być zadeklarowana jako `melody main() {...}` w pliku startowym (tym który jest uruchamiany), musi być dokładnie 1 taka deklaracja dla całego programu (nie może wystąpić w innych importowanych plikach)

```
melody moja_melodia(Note nuta, int dlugosc) {
    Note n2 = nuta + 4;

    PLAY nuta n2 nuta n2 nuta n2 dlugosc;
}

melody main() {
    SET INSTRUMENT = GUITAR;
    PLAY moja_melodia(C2, 600);
...
melody main() {
    ...
    PLAY C1 up:i;       // odwołanie do zmiennej globalnej
}
```

9. **Wywoływanie funkcji i zwrot wartości**\
...
melody main() {
    Note n;
    PLAY mel1() OUT n;    // n będzie miało wartość C2
    PRINT(n);
}
```
...
11. **Operacje na zmiennych i operatory**
...
melody main() {
    if(5 > 6) {
        ...
        // w tym przypadku nie, bo zagrał 2 warunek else-if'a
        PLAY C4 500;
    }
}
```
...
18. **Importowanie plików własnych:**\
...
melody main() {
    ...
    // zakłada istnienie w zaimportowanych plikach bezargumentowych funkcji analogicznych do ich nazw
}
```